...
var /*canvas = document.getElementById('boceto'),
    ...
    /*,
    btn_Hoja = document.getElementById('seleccionarHoja')*/
    /*,
...
//btn_Hoja.addEventListener('click', function(){cargarHoja()});
...
function insertarPuntoEnTrazo(miTrazoR, punto, recorridoX, recorridoY, miPuntero, tipoTuberia){
  ...
  for(rp = grafo[miPuntero[0]].length-1; rp >= miPuntero[8]; rp--){
    ...
    if(grafo[miPuntero[0]][rp] == 'S'){
      grafo[miPuntero[0]][cDown] = 'S';
    }
    cDown--;
  }
  ...
  if(markedButton){
    ...
    if(punteroDeDibujo.length == 0)
      ...
      if(modo == 'dibujo'){
        ...
        //console.log('PUNTO A BUSCAR: ' + miPunto)
        //var miNuevaPosicion = obtenerPosiciones(numHoja, tolerancia, px, py)
        var miTrazoParaInsertar = obtenerDatosTrozoAPartirDe(punteroDeDibujo);

        console.log('TRAZO A RECORRER');
        console.log(miTrazoParaInsertar);
        ...
        insertarPuntoEnTrazo(miTrazoParaInsertar, miNuevoPunto, miNuevoPunto.x - miPunto[0], miNuevoPunto.y - miPunto[1], punteroDeDibujo, typeTub);
...
function marcarGrafoDeLaLista(numeroGrafo){
  var miGrafoPos = numeroGrafo.substring(11, numeroGrafo.length);
  ...
  if(misAutoCoords.length == 2){
    ...
    marcarGrafo(miNumeroLienzo, miGrafoPos, miPosicion[2], modo)
  }
  else{
...
function obtenerRectaParaEditar(e){
  var misCoordenadas = obtenerCoordenadasLienzo(e);
  var miLienzo = obtenerPosicionLienzoMarcado();
  ...
  if(punteroDeEdicion.length > 0)
    marcarRecta(punteroDeEdicion[1], punteroDeEdicion[0], punteroDeEdicion[6], modo);
...
//svg_lienzo.addEventListener('click', obtenerPosicionInterseccion, false);
//svg_lienzo.addEventListener('dblclick', obtenerPosicionParaDibujar, false);

//svg_lienzo.addEventListener('mousedown', obtenerPosicionParaDibujar, false);

//svg_lienzo.addEventListener('click', prueba, false);
//svg_lienzo.addEventListener('click', seleccionarTrozo, false);
//svg_lienzo.addEventListener('mousedown', obtenerPosicionInterseccion, false);
...
//svg_lienzo.addEventListener('mouseout', desmarcarPuntoPartida, false);//OJO
